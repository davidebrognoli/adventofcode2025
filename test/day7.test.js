import { parseGrid } from '../src/utils.js';
import { solvePart1, solvePart2 } from '../src/day7/index.js';
import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';
import { test, expect } from 'vitest';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

const realInput = fs.readFileSync(
  path.resolve(__dirname, '../src/day7/input.txt'),
  'utf-8'
);

const testInput = `.......S.......
...............
.......^.......
...............
......^.^......
...............
.....^.^.^.....
...............
....^.^...^....
...............
...^.^...^.^...
...............
..^...^.....^..
...............
.^.^.^.^.^...^.
...............`;

// Test for Part 1
test('Example case for Part 1', () => {
  const input = parseGrid(testInput);
  expect(solvePart1(input)).toBe(21);
});

test('Real case for Part 1', () => {
  const input = parseGrid(realInput);
  expect(solvePart1(input)).toBe(1594);
});

// Test for Part 2
test('Example case for Part 2', () => {
  const input = parseGrid(testInput);
  expect(solvePart2(input)).toBe(40);
});

test('Real case for Part 2', () => {
  const input = parseGrid(realInput);
  expect(solvePart2(input)).toBe(15650261281478);
});
